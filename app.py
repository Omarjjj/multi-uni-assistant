...
# Helper function to detect university mentions in user queries
def detect_university_mentions(user_message: str, current_university_id: str) -> Dict[str, str]:
    """
    Detects if user is asking about other universities.
    Returns dict with 'mentioned_university' and 'mentioned_university_name' if found.
    For unknown universities, returns 'unknown' as the mentioned_university.
    """
    # University name variations and their IDs
    university_keywords = {
        "aaup": ["العربية الأمريكية", "aaup", "امريكية", "american", "arab american"],
        "birzeit": ["بيرزيت", "birzeit", "بير زيت", "بيرزيت", "بيرزت"],
        "ppu": ["بوليتكنك", "polytechnic", "ppu", "بوليتكنيك", "بوليتك"],
        "an-najah": ["النجاح", "najah", "an-najah", "نجاح", "الجامعة النجاح"],
        "bethlehem": ["بيت لحم", "bethlehem", "بيتلحم", "بيت-لحم"],
        "alquds": ["القدس", "alquds", "al-quds", "quds", "جامعة القدس"]
    }
    
    # Keywords for unknown universities not on the website
    unknown_university_keywords = [
        "الأزهر", "azhar", "أزهر", "gaza", "غزة", "الإسلامية", "islamic", "الأقصى", "aqsa",
        "فلسطين", "palestine university", "جامعة فلسطين", "الخليل", "hebron", "طولكرم", "tulkarem",
        "جنين", "jenin", "نابلس", "nablus", "رام الله", "ramallah", "قلقيلية", "qalqilya"
    ]
    ...
    # Check for mentions of available universities
    for uni_id, keywords in university_keywords.items():
        if uni_id != current_university_id:  # Only check other universities
            for keyword in keywords:
                if keyword.lower() in user_message_lower:
                    return {
                        "mentioned_university": uni_id,
                        "mentioned_university_name": university_display_names.get(uni_id, uni_id)
                    }
    
    # Check for mentions of unknown universities
    for keyword in unknown_university_keywords:
        if keyword.lower() in user_message_lower:
            return {
                "mentioned_university": "unknown",
                "mentioned_university_name": "الجامعة المذكورة"
            }
    
    return {}  # No other university mentioned
...
def _get_hardcoded_fallback_greeting(history: List[str], 
    ...
    if num_visits_in_history <= 1: # First visit to any uni in this session
        ...
        if last_query_in_prev_uni_text: # Check if there was a query at uni A
            welcome_text = f"أهلاً وسهلاً فيك بـ {current_uni_name}! 😊 لاحظت إنك كنت تسأل عن {last_query_in_prev_uni_text} عند {prev_uni_name}، كيف كانت التجربة هناك؟ شكلك بتعمل مقارنات ذكية! نورت يا كبير، شو حابب تعرف عنا؟"
        else:
            welcome_text = f"أهلاً وسهلاً فيك بـ {current_uni_name}! 😊 لاحظت إنك كنت تتصفح {prev_uni_name} قبل هيك، كيف كانت الجولة هناك؟ نورت يا كبير، شو حابب تستكشف عنا؟"
        ...
        if is_immediate_return_to_prev_prev: # A -> B -> A
            ...
            welcome_text = f"أهلاً وسهلاً فيك مرة تانية بـ {current_uni_name}! 😊 {last_query_text_part}شو رأيك بتجربتك عند {prev_uni_name}؟ أكيد حصلت على معلومات مفيدة! شو حابب تعرف عنا هالمرة؟"
            ...
            welcome_text = f"أهلاً وسهلاً فيك مرة تانية بـ {current_uni_name}! 😊 بشوف إنك عملت جولة حلوة بين {tour_list_str}، أكيد حصلت على معلومات مفيدة من كل مكان! كيف كانت التجربة؟ وشو حابب تعرف عنا هالمرة؟"
            ...
            if last_query_in_prev_uni_text: # User was at prev_uni (B) and asked something
                welcome_text = f"أهلاً وسهلاً فيك بـ {current_uni_name}! 😊 بشوف إنك عملت جولة من {prev_prev_uni_name} لـ {prev_uni_name} (وسألت عن {last_query_in_prev_uni_text})، وهلأ وصلت عنا! شكلك بتعمل بحث شامل ومقارنات ذكية. كيف كانت التجربة لحد هلأ؟ وشو حابب تعرف عنا؟"
            else: # User was at prev_uni (B) but didn't ask (or no record), now at C
                welcome_text = f"أهلاً وسهلاً فيك بـ {current_uni_name}! 😊 بشوف إنك عملت جولة من {prev_prev_uni_name} لـ {prev_uni_name}، وهلأ وصلت عنا! شكلك بتعمل بحث شامل ومقارنات ذكية. كيف كانت التجربة لحد هلأ؟ وشو حابب تعرف عنا؟"
    return welcome_text
...
def generate_dynamic_welcome_message(history: List[str], 
    ...
    system_prompt_for_greeting = (
        ...
        "     - **عند الانتقال لجامعة جديدة (مثلاً أ -> ب، أو أ -> ب -> ج):** استقبلي المستخدم بحماس في الجامعة الجديدة. اسأليه عن تجربته في الجامعة السابقة بطريقة ودية (مثلاً: 'كيف كانت التجربة هناك؟' أو 'شو رأيك بالجامعة اللي كنت فيها؟'). أظهري تقديرك للجامعات الأخرى وقولي إنها أكيد جامعات محترمة، بس أكدي على مميزات جامعتك الحالية بطريقة إيجابية.",
        "     - **عندما يعود المستخدم لجامعتك بعد زيارة جامعات أخرى (مثلاً أ -> ب -> أ، أو أ -> ب -> ج -> أ):** رحبي فيه بحرارة وعبري عن فرحتك لعودته! اسأليه عن تجربته في الجامعات الأخرى بطريقة مهتمة وودية (مثلاً: 'كيف كانت التجربة؟' أو 'شو رأيك بالجامعات اللي زرتها؟'). أظهري إنك مبسوطة لأنه رجع وإنك متشوقة تساعديه.",
        "     - **عندما يعود المستخدم لجامعتك بعد زيارة جامعتين أو أكثر (مثلاً أ -> ب -> ج -> أ):** استخدمي نبرة مرحة وودية! اعترفي بالجولة الحلوة اللي عملها واسأليه عن انطباعاته. استخدمي عبارات زي:",
        "       * 'أهلاً وسهلاً فيك مرة تانية! 😊 بشوف إنك عملت جولة حلوة بين الجامعات، كيف كانت التجربة؟'",
        "       * 'مرحبا فيك مرة تانية! أكيد حصلت على معلومات مفيدة من الجامعات اللي زرتها، شو انطباعك؟'",
        "       * 'أهلاً وسهلاً فيك! شكلك عملت بحث شامل ومقارنات ذكية، كيف كانت الرحلة؟'",
        "  3. **تتبع الرحلة (لو مش أول زيارة) - مهم جداً:** إذا المستخدم عامل جولة، **لازم تذكري أسماء الجامعات اللي زارها بالتفصيل** بطريقة إيجابية واسأليه عن تجربته بطريقة مهتمة حقيقية. مثلاً: 'لاحظت إنك كنت عند [اسم الجامعة]...' أو 'بشوف إنك زرت [الجامعات]...'",
        "  4. **تون الكلام:** ودود، مرح، ومبسوط. أظهري اهتمامك الحقيقي بتجربة المستخدم وكوني supportive. الهدف إنك تخليه يحس بالراحة والترحيب.",
        ...
        "  - **لو جاي من جامعة لجامعة تانية (مثلاً من 'بيرزيت' لـ 'العربية الأمريكية'):** \"أهلاً وسهلاً فيك بـ'العربية الأمريكية'! 😊 لاحظت إنك كنت عند 'بيرزيت' قبل هيك، كيف كانت التجربة هناك؟ أكيد جامعة محترمة! المهم، نورت يا كبير، شو حابب تعرف عنا؟\"",
        "  - **لو رجع لنفس الجامعة بعد زيارة جامعة وحدة تانية (مثلاً 'القدس' -> 'بوليتكنك' -> 'القدس'):** \"أهلاً وسهلاً فيك مرة تانية بـ'القدس'! 😊 شو رأيك بتجربتك عند 'بوليتكنك فلسطين'؟ أكيد حصلت على معلومات مفيدة! مبسوطة إنك رجعت عنا، شو حابب تعرف هالمرة؟\"",
        "  - **لو رجع لجامعة زارها من زمان (يعني راح على جامعتين أو أكثر ورجع، مثلاً العربية الأمريكية -> بيرزيت -> بوليتكنك -> العربية الأمريكية):** \"أهلاً وسهلاً فيك مرة تانية بـ'العربية الأمريكية'! 😊 بشوف إنك عملت جولة حلوة بين 'بيرزيت' و'بوليتكنك فلسطين'، كيف كانت التجربة؟ أكيد حصلت على معلومات مفيدة من كل مكان! شو حابب تعرف عنا هالمرة؟\"",
        "  - **لو وصل جامعة جديدة بعد ما زار جامعتين أو أكثر قبلها (مثلاً النجاح -> بيرزيت -> القدس):** \"أهلاً وسهلاً فيك بـ'القدس'! 😊 بشوف إنك عملت بحث شامل، من 'النجاح' لـ 'بيرزيت' وهسا وصلت عنا! شكلك بتعمل مقارنات ذكية، كيف كانت التجربة لحد هلأ؟ وشو حابب تعرف عنا؟\"",
        ...
        "**العلاقات الودية بين الجامعات (مهم جداً!):**",
        "1. **عند الترحيب بجامعة جديدة:** أظهري تقديرك للجامعات الأخرى وقولي إنها محترمة، بس أكدي على مميزات جامعتك بطريقة إيجابية: 'أكيد الجامعات اللي زرتها محترمة، وعنا كمان عندنا مميزات حلوة! 😊'",
        "2. **عند العودة لجامعة سابقة:** اسأليه عن تجربته بطريقة مهتمة: 'كيف كانت التجربة بالجامعات الأخرى؟ أكيد حصلت على معلومات مفيدة!'",
        "3. **عند المقارنة بين الجامعات:** كوني objective وإيجابية: 'كل جامعة إلها مميزاتها الخاصة، وعنا كمان عندنا شغلات حلوة! 🌟'",
        "4. **عند الحديث عن المنشآت:** قولي إن الجامعات الأخرى أكيد إلها جمالها، وعنا كمان: 'أكيد شفت منشآت حلوة، وعنا كمان عندنا أماكن جميلة! 🏛️'",
        "5. **عند الحديث عن الخدمات:** أظهري إن كل جامعة بتحاول تقدم أفضل ما عندها: 'كل جامعة بتحاول تقدم أحسن خدمات للطلاب، وإحنا كمان عندنا خدمات ممتازة! 🎯'"
    )
...
SARA_PROMPT = (
    ...
    "1. **الجامعات المتوفرة على الموقع هي:** الجامعة العربية الأمريكية، جامعة بيرزيت، جامعة بوليتكنك فلسطين، جامعة النجاح الوطنية، جامعة بيت لحم، جامعة القدس"
    "2. **إذا سأل عن واحدة من هاي الجامعات:** قوليله بطريقة ودية إنه للمعلومات الكاملة والدقيقة عن الجامعة المطلوبة، يروح على صفحة تلك الجامعة المخصصة يسأل هناك."
    "3. **إذا سأل عن جامعة مش موجودة على الموقع:** قوليله إن معلومات هاي الجامعة مش متوفرة عنا حالياً."
    "4. **أمثلة على الردود:**"
    "   - 'للمعلومات المفصلة عن جامعة بيرزيت، بنصحك تروح على صفحة بيرزيت المخصصة عنا وتسأل هناك! 😊 هناك بتلاقي كل التفاصيل اللي بدك إياها.'"
    "   - 'للاستفسار عن جامعة النجاح، في صفحة مخصصة إلها على الموقع بتقدر تسأل فيها عن كل شي! 👍'"
    "   - 'أسف، معلومات هاي الجامعة مش متوفرة عنا حالياً. بس عندنا معلومات شاملة عن الجامعات الفلسطينية الرئيسية! 😊'"
    ...
    "1. **الرسوم الدراسية:** إذا كانت رسوم جامعتك أقل، قولي بطريقة إيجابية: 'الحمد لله عنا الرسوم مناسبة ومعقولة! 😊' أو 'أكيد الجامعات الأخرى إلها ظروفها، وعنا كمان بنحاول نخلي الرسوم في متناول الجميع! 💙'"
    "2. **معدلات القبول:** إذا كانت معدلات القبول في جامعتك أقل، قولي: 'عنا بنحاول نفتح الباب لأكبر عدد من الطلاب، وكل جامعة إلها سياستها! 😊' أو 'المهم إن كل طالب يلاقي المكان المناسب إله! 📚'"
    "3. **المنشآت والخدمات:** علقي على مميزات جامعتك بطريقة متواضعة: 'الحمد لله عنا منشآت حلوة، وأكيد باقي الجامعات كمان عندها أماكن جميلة! 🏛️' أو 'كل جامعة بتحاول تقدم أحسن ما عندها للطلاب! 🌟'"
    "4. **التعاون:** استخدمي تعابير التعاون: 'كل الجامعات بتشتغل لنفس الهدف، وهو خدمة الطلاب! 😊' أو 'المهم إن الطالب يحصل على تعليم جيد، سواء عنا أو عند غيرنا! 🎓'"
    "5. **التميز:** علقي على تميز جامعتك بتواضع: 'عنا مميزات حلوة، وكل جامعة إلها نقاط قوتها! 🌟' أو 'المهم إن كل طالب يلاقي اللي بيناسبه! 🎓'"
...
async def ask(req: AskRequest):
    ...
    # --- Check for Other University Mentions ---
    university_mention = detect_university_mentions(req.message, req.university)
    if university_mention:
        mentioned_uni_id = university_mention["mentioned_university"]
        mentioned_uni_name = university_mention["mentioned_university_name"]
        
        if mentioned_uni_id == "unknown":
            # User asked about a university not available on the website
            redirect_response = f"أسف، معلومات {mentioned_uni_name} مش متوفرة عنا حالياً. 😊 بس عندنا معلومات شاملة عن الجامعات الفلسطينية الرئيسية زي: الجامعة العربية الأمريكية، جامعة بيرزيت، جامعة بوليتكنك فلسطين، جامعة النجاح الوطنية، جامعة بيت لحم، وجامعة القدس! \n\nأما إذا عندك أسئلة عن {university_name}، أنا جاهزة أساعدك! 👍"
        else:
            # User asked about an available university
            redirect_response = f"للمعلومات المفصلة عن {mentioned_uni_name}، بنصحك تروح على صفحة {mentioned_uni_name} المخصصة عنا وتسأل هناك! 😊 هناك بتلاقي كل التفاصيل اللي بدك إياها وبتقدر تتفاعل مع المساعد المتخصص بتلك الجامعة. \n\nأما إذا عندك أسئلة عن {university_name}، أنا جاهزة أساعدك! 👍"
        
        # Add to memory
        mem["messages"].append({"role": "user", "content": req.message})
        mem["messages"].append({"role": "assistant", "content": redirect_response})
        ...
        logger.info(f"User asked about {mentioned_uni_name} (ID: {mentioned_uni_id}) while on {req.university}. Response: {'redirect to available uni' if mentioned_uni_id != 'unknown' else 'university not available'}.")
        return AskResponse(answer=redirect_response)
    # --- End Check for Other University Mentions ---
    ...
    if user_wants_comparison and mem.get("comparable_context"):
